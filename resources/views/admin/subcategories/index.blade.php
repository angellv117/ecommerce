<x-admin-layout :breadcrumb="[
    ...
    <x-slot name="actionButton">
        <button class="custom-button custom-button-green" type="button"
            onclick="window.location.href = '{{ route('admin.subcategories.create') }}'">
            <p class="text-white hidden  md:block">Agregar subcategoría</p>
    ...
    @if ($subcategories->isEmpty())
        <div class="flex items-center p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50    "
            ...
            <div>
                <span class="font-medium">Info alert!</span> Aun no hay subcategorías de productos.
            </div>
        </div>
    @else
                            ...
                            @foreach ($subcategories as $subcategory)
                                <tr class="bg-white border-b     border-gray-200">
                                    ...
                                    <td class="px-6 py-4 flex items-center justify-center space-x-2">
                                        <button type="button"
                                            x-on:click="window.location.href = '{{ route('admin.subcategories.edit', $subcategory->id) }}'"
