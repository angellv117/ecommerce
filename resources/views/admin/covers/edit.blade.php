<x-admin-layout :breadcrumb="[
    ...
    <div class="">
        <div class="bg-white rounded-xl shadow-sm">
            <form action="{{ route('admin.covers.update', $cover) }}" method="POST" class="card-custom"
                ...
                <div class="mb-2">
                    ...
                    <x-input name="title" label="Título"
                        ...
                        placeholder="Ingrese el título de la portada" value="{{ old('title', $cover->title) }}" />
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-2">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input datepicker type="date" name="start_date" id="start_date"
                                value="{{ old('start_date', $cover->start_date->format('Y-m-d')) }}"
                    ...
                    <!-- Fecha de fin -->
                    <div>
                        <x-label for="end_date" class="text-sm font-bold mb-1 text-gray-700">Fecha de fin
                            (Opcional)</x-label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                <svg class="w-4 h-4 text-gray-500" fill="currentColor" viewBox="0 0 20 20"
                            ...
                            <input datepicker type="date" name="end_date" id="end_date"
                                value="{{ old('end_date', $cover->end_date ? $cover->end_date->format('Y-m-d') : '') }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5"
                                placeholder="Seleccionar fecha">
                        </div>
                    </div>
                ...
                <!-- Estado Activo -->
                <div class="mb-2" x-data="{ isActive: {{ old('is_active', $cover->is_active) ? 'true' : 'false' }} }">
                    ...
                    <label class="inline-flex items-center cursor-pointer">
                        ...
                        <input type="checkbox" value="1" class="sr-only peer" name="is_active"
                            {{ old('is_active', $cover->is_active) ? 'checked' : '' }}>
                        <div
                ...
                @if ($cover->image_path)
                    <div class="mb-4">
                        <x-label class="text-sm font-bold mb-3 text-gray-700">Imagen actual</x-label>
                        <img src="{{ $cover->image_path }}" name="image_path" alt="Portada actual"
                            class="aspect-[3/1] object-cover w-full rounded-lg shadow-md border border-gray-200">
                    </div>
                @endif
                ...
                <div x-data="{
                    ...
                    <x-label
                        class="text-sm font-bold mb-3 text-gray-700">{{ $cover->image_path ? 'Cambiar imagen de portada' : 'Imagen de portada' }}</x-label>

                    <!-- Vista previa de la imagen subida actualmente -->
                    <div x-show="imagePreview" class="mb-4 flex flex-col items-center">
                        ...
                        <span class="text-sm text-gray-500 mt-2">Vista previa de la nueva imagen</span>
                    </div>
                ...
                <!-- Botones de acción -->
                <div class="flex justify-start space-x-3 border-t mt-4 border-gray-100 pt-4">
    ...
    <form id="delete-form" action="{{ route('admin.covers.destroy', $cover) }}" method="POST">
        @csrf
        @method('DELETE')
    </form>
...
@push('scripts')
    <script>
        function confirmDelete() {
            Swal.fire({
                title: '¿Estás seguro?',
                text: 'Esta acción eliminará la portada',
                icon: 'warning',
